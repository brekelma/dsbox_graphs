...
class SDNE(UnsupervisedLearnerPrimitiveBase[Input, Output, SDNE_Params, SDNE_Hyperparams]):
    ...
    def _parse_inputs(self, inputs : Input, return_all = False, use_old=True):	 
	try:
	...
	#    pass
		...
		sources = d3m_DataFrame(edges[:,0])
		dests = d3m_DataFrame(edges[:,1])
	    else:
		...
		try:
		    ...
		    f.write("\n")
		    f.write(str("tried graphs[0] but ", e))
		try:
		    f.write(graphs)
		except:
		    pass
		f.write(str('\n'))
		f.write(str(type(sources)))
		f.write(str(type(dests)))
		f.write(str("\n"))
		try:
		    f.write(sources.values)
		except:
		    pass
	...
	# else: loading networkx via jhu_load
	...
	to_fit = np.sort(np.concatenate([sources.values, dests.values], axis = -1).ravel()) #.astype(np.int32).ravel())
	...
	if self.hyperparams['return_list']:
		...
		with open('sdne_log.csv', 'a') as f:
		    for i in range(len(list(learn_df))):
			f.write(str(list(learn_df)[i]))
