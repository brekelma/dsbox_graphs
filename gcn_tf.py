...
def mixhop_layer(x, sparse_adj, adj_powers, dim_per_power):
...
class GCN(SupervisedLearnerPrimitiveBase[Input, Output, GCN_Params, GCN_Hyperparams]):
        ...
        def fit(self, *, timeout : float = None, iterations : int = None) -> None:
                ...
                os_training = tf.placeholder_with_default(True, [], name='is_training')
                
                sparse_adj = dataset.sparse_adj_tensor() 
                h = tf.sparse_tensor_to_dense(x_inp)

                mix = mixhop_layer() #(x, sparse_adj, adj_powers, dim_per_power)
