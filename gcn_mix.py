...
class GCN(SupervisedLearnerPrimitiveBase[Input, Output, GCN_Params, GCN_Hyperparams]):
	...
	def pred(self, adj_input, feature_input, embedding = False, _slice = False):
		fd = {adj_input: self._adj,       
			feature_input: self._input}#,
	...
	def fit(self, *, timeout : float = None, iterations : int = None) -> None:
		...
		self.adj_input = tf.sparse_placeholder(tf.float32)
		self.feature_input = tf.sparse_placeholder(tf.float32)
		...
		if self._extra_fc is not None:
			self.embedding = keras.layers.Dense(self._extra_fc, activation = self._act)(self.embedding)

		label_act = 'softmax' if self._label_unique > 1 else 'sigmoid'
		self.y_pred = keras.layers.Dense(self._label_unique, activation = label_act, name = 'y_pred')(self.embedding)
		...
		slice_loss = keras.layers.Lambda(loss_fun, arguments = {'function': loss_function, 'first': self._num_labeled_nodes})([y_true_slice, self.y_pred_slice])
		
		full_loss = keras.layers.Lambda(assign_scattered)([slice_loss, self.y_pred, self.inds])
		...
		def step(lr=None, columns=None):
		    ...
		    train_preds, loss_value, _ = self.sess.run((y_pred_slice, slice_loss, train_op), feed_dict)
		...
		for h_i in range(len(h_dims)):
			...
			for mode in range(modes):
				for k in range(mix_hops+1):
					...
					act = keras.layers.Dense(h_dims[h_i], activation = self._act, name='w'+str(k)+'_'+str(mode)+'_'+str(h_i))(pre_w)
					act_k.append(act)
